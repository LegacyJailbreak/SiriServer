            ...
            with open(pluginPath+"/"+line+".py", "r") as fd:
                for tline in fd:
                    ...
                    if tline.startswith("@register(\"de-DE\","):
                        ...
                        tline = tline.replace('@register','').replace('(','').replace(')','').replace('\"','').replace('.*','... ').replace('en-US,','').replace('[a-zA-Z0-9]+','').replace('\w','').replace('|','/').replace("res['setAlarm']['en-US']",'... set... alarm for... am/pm... called/named/labeled... ').replace("res['setTimer']['en-US']",'... timer... seconds/minutes/hours').replace("res['pauseTimer']['en-US']",'pause/freeze/hold... timer').replace("res['resetTimer']['en-US']",'... cancel/reset/stop... timer').replace("res['resumeTimer']['en-US']",'... resume/thaw/continue... timer').replace("res['showTimer']['en-US']",'... show/display/see... timer')
                        tline = tline.replace('+','').replace('?','').replace('[]','... ').replace('[','').replace(']','').replace(':','').replace('P<name>','').replace('P<type>','').replace('[a-z]','... ').replace('*.','... ').replace('*','... ').replace('\n\n','')
