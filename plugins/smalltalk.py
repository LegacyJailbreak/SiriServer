...
class smalltalk(Plugin):
    ...
    def st_fuck(self, speech, language):  
        if language == 'de-DE':
            ...
            self.say(u"说这种话，小心被和谐啊{0}！".format(self.user_name()))
        else:            
            self.say(u"Your language {0}!".format(self.user_name()))
        self.complete_request()   
    ...
    def st_hello(self, speech, language):
        if language == 'de-DE':
            ...
            self.say(u"您好{0}，我是厌烦羊的 Siri 服务！".format(self.user_name()))
        else:
            self.say(u"Greetings {0}, I am Siri service from Linus Yang!".format(self.user_name()))
        self.complete_request()
    ...
    def st_marry_me(self, speech, language):
        if language == 'de-DE':
            ...
            rep = [u"可惜我对你没感觉。", u"嗯，谢谢。可是我已经和你朋友的 iPhone 恋爱了。"]
            self.say(random.choice(rep))
        else:
