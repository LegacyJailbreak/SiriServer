...
class HandleConnection(ssl_dispatcher):
    __not_recognized =  {"de-DE": u"Entschuldigung, ich verstehe \"{0}\" nicht.", "en-US": u"Sorry, I don't understand '{0}'.", "fr-FR": u"Désolé je ne comprends pas ce que \"{0}\" veut dire.", "zh-CN": u"抱歉，我无法理解“{0}”。"}
        ...
        while self.hasNextObj():
            ...
            if reqObject != None:
                ...
                if ObjectIsCommand(reqObject, StartSpeechRequest) or ObjectIsCommand(reqObject, StartSpeechDictation):
                    ...
                    self.process_recognized_speech({u'hypotheses': [{'confidence': 1.0, 'utterance': (reqObject['properties']['utterance']).decode("utf-8").lower()}]}, reqObject['aceId'], False)
